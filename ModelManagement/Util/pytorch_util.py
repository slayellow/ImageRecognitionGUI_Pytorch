...
def set_conv(in_channel, out_channel, kernel=3, strides=1, padding=1, dilation=1 ):
    return nn.Conv2d(in_channel, out_channel, kernel_size=kernel, stride=strides, padding=padding, dilation=dilation)


def set_detphwise_conv(in_channel, out_channel, kernel=3, strides=1, padding=1, dilation=1):
    return nn.Conv2d(in_channel, out_channel, kernel_size=kernel, stride=strides, padding=padding, groups=in_channel, dilation=dilation)


def set_pointwise_conv(in_channel, out_channel, kernel, strides=1, padding=0, dilation=1):
    return nn.Conv2d(in_channel, out_channel, kernel_size=kernel, stride=strides, padding=padding, dilation=dilation)
